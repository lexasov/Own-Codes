...
double cubickernel(double r,double h1);
...
for( n=0;n<npart;n++){
  ...
  if(n%100000==0){ std::cout << "particle " << n << " of " << npart << std::endl;}
        ...
        for( i=min_intz; i<=max_intz;i++){
          if(i<0){  zindex=i+npixels;  }
          else if(i>=npixels){ zindex=i-npixels; }
          else { zindex=i; }
          ...
          for( j=min_intx;j<=max_intx;j++){
            if(j<0){  xindex=j+npixels;  }
          else if(j>=npixels){ xindex=j-npixels; }
          else { xindex=j; }
        ...
        for(k=min_inty;k<=max_inty;k++){
            if(k<0){  yindex=k+npixels;  }
          else if(k>=npixels){ yindex=k-npixels; }
          else { yindex=k; }
